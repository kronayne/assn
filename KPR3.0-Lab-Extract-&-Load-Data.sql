...
-- MAGIC %md
-- MAGIC path = "${da.paths.datasets}/raw/events-kafka"

-- COMMAND ----------

CREATE TABLE events_json
  LOCATION "${da.paths.working_dir}/lab/external"
  AS SELECT cast(key as binary), cast(offset as bigint), cast(`partition` as int), cast(`timestamp` as bigint), cast(topic as string), cast(value as binary)
...
-- Looking at just the top few results from each table, all looks the same
SELECT * FROM events_raw LIMIT 2

-- COMMAND ----------

SELECT * FROM events_json LIMIT 2

-- COMMAND ----------

-- By subtracting all of one of the outputs from all of the other, I can see if there is anything in one table not in the other. There isn't (the query returns nothing)! Meaning that everything inserted as it should have
...
-- MAGIC %md
-- MAGIC ${da.paths.datasets}/raw/item-lookup
